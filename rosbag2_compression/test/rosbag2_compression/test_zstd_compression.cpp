// Copyright 2019 Amazon.com, Inc. or its affiliates. All Rights Reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include <gmock/gmock.h>

#include <fstream>
#include <string>
#include <rclcpp/rclcpp.hpp>

#include "rosbag2_storage/filesystem_helper.hpp"
#include "rosbag2_test_common/temporary_directory_fixture.hpp"

#include "rosbag2_compression/zstd_compression.hpp"

using namespace testing;  // NOLINT
using namespace rosbag2_storage;  // NOLINT
using namespace rosbag2_test_common;  // NOLINT

class FilesystemHelperFixture : public TemporaryDirectoryFixture {};

void create_garbage_file(const std::string & uri)
{
  std::ofstream out(uri);
  for (int i = 0; i < 984467; i++) {
    out << "GarbageGarbageGarbageGarbage";
  }
  out << "test";
  out.close();
}

TEST_F(FilesystemHelperFixture, zstd_compress_file_uri)
{
  rclcpp::init(0, nullptr);
  // 27MiB file
  auto uri = FilesystemHelper::concat({temporary_dir_path_, "file1.txt"});
  create_garbage_file(uri);

  auto zstd_compressor = rosbag2_compression::ZstdCompressor();
  auto compressed_uri = zstd_compressor.compress_uri(uri);
  rclcpp::shutdown();

  auto expected_compressed_uri = uri + "." + zstd_compressor.get_compression_identifier();
  auto uncompressed_file_size = FilesystemHelper::get_file_size(uri);
  auto compressed_file_size = FilesystemHelper::get_file_size(compressed_uri);

  EXPECT_EQ(compressed_uri, expected_compressed_uri);
  EXPECT_LT(compressed_file_size, uncompressed_file_size);

}

TEST_F(FilesystemHelperFixture, zstd_decompress_file_uri)
{
  rclcpp::init(0, nullptr);
  // 27MiB file
  auto uri = FilesystemHelper::concat({temporary_dir_path_, "file1.txt"});
  create_garbage_file(uri);

  auto zstd_compressor = rosbag2_compression::ZstdCompressor();
  auto compressed_uri = zstd_compressor.compress_uri(uri);

  auto expected_compressed_uri = uri + "." + zstd_compressor.get_compression_identifier();
  auto uncompressed_file_size = FilesystemHelper::get_file_size(uri);
  auto compressed_file_size = FilesystemHelper::get_file_size(compressed_uri);

  EXPECT_EQ(compressed_uri, expected_compressed_uri);
  EXPECT_LT(compressed_file_size, uncompressed_file_size);

  auto zstd_decompressor = rosbag2_compression::ZstdDecompressor();
  auto decompressed_uri = zstd_decompressor.decompress_uri(compressed_uri);
  rclcpp::shutdown();
  uncompressed_file_size = FilesystemHelper::get_file_size(decompressed_uri);

  EXPECT_EQ(decompressed_uri, uri);
  EXPECT_GT(uncompressed_file_size, compressed_file_size);
}
